# Nemotron ASR + Diarization System (Windows)

This repository contains a modular, production-grade Speaker Diarization system built for local execution on Windows using the **NVIDIA Nemotron 3.5 ASR** streaming model (ONNX INT4 quantized) for transcription and an ONNX-based backend for embedding extraction and clustering.

**Key Features:**
- Streaming ASR via `onnxruntime-genai` with **word-level timestamp support** (560ms granularity from streaming chunk positions)
- 120s batch chunks for efficient full-file transcription
- Built-in diarization pipeline (VAD → embedding → clustering → voiceprint matching)
- Real-time dual-channel streaming via WebSocket
