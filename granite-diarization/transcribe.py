"""
Async audio transcription client with Granite SAA support.

When segments from the SAA diarization include pre-computed `text`,
the client skips the separate /transcribe/paths ASR call.
"""
...
class TranscriptionClient:
    ...
    async def diarize_path(self, wav_path: str) -> tuple[list[dict], dict]:
        """
        Run hybrid SAA diarization via /diarize/path.
        Returns (segments, profiles) where segments may include pre-computed 'text'.
        """
        try:
            headers = {}
            if self.config.api_key:
            ...
            log.warn("Failed to get diarization: Connection timed out")
            return [], {}
    ...
    async def transcribe_batch(
        ...
        async with self.semaphore:
            return await self._transcribe_with_retry(wav_paths)
            ...
            try:
                async with self._get_session() as session:
                    async with session.post(
                        ...
                        json={"wav_paths": wav_paths, "language": "en"}
                    ) as resp:
                        resp.raise_for_status()
                        data = await resp.json()
...
def refactor_and_detect_overlaps(diarize_segments: list[dict], min_duration: float,
    ...
    for seg in sorted_segments[1:]:
        same_speaker_and_continuous = (
            seg['speaker'] == current['speaker'] and
            (seg['start'] - current['end']) <= merge_gap
        )
        if same_speaker_and_continuous:
...
async def transcribe_file(
    ...
    try:
        ...
        log.info("Running SAA diarization on the full audio...")
        diarize_segments, speaker_profiles = await client.diarize_path(temp_wav)

        # Update voiceprints.json with new speakers
        ...
        if not diarize_segments:
            log.warn("Diarization returned no segments. Assuming single speaker.", file=p.name)
            total_dur = get_total_duration(temp_wav)
            diarize_segments = [{"start": 0.0, "end": total_dur, "speaker": "SPEAKER", "text": ""}]
            speaker_profiles = {}

        # Refine segments and check if SAA already provided text
        ...
        for batch_idx in range(0, len(segments), config.batch_size):
            ...
            for j, seg in enumerate(batch):
                if len(seg) >= 6:
                    start, end, speaker, confidence, alternatives, text = seg[0], seg[1], seg[2], seg[3], seg[4], seg[5]
                elif len(seg) >= 5:
                    start, end, speaker, confidence, alternatives = seg[0], seg[1], seg[2], seg[3], seg[4]
                    text = ""
                elif len(seg) >= 4:
                ...
                if text and text.strip():
                    # SAA already provided text — no need for ASR call
                    has_text_chunks.append((start, end, speaker, confidence, alternatives, text))
                else:
                    needs_asr_chunks.append(j)
            ...
            # Write segments that already have text from SAA
            for start, end, speaker, confidence, alternatives, text in has_text_chunks:
                if text.strip():
        ...
        if diarize_segments:
            for seg in diarize_segments:
                ...
                if current_start > speaker_stats[spk]["last_end"] + 0.5:
            ...
            for spk in speaker_stats:
                ...
                del speaker_stats[spk]["last_end"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Transcribe audio/video files using Granite ASR server",
