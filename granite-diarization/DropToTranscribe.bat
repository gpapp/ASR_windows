...
if "%~1"=="" (
    ...
    echo.
    echo Using Granite SAA for speaker-attributed transcription + ECAPA-TDNN for voiceprint matching.
    pause
    exit /b 0
)
...
if %SERVER_STATUS% equ 1 (
    ...
    start "Granite Transcribe Server" /min cmd /c "cd /d "!LAUNCH_DIR!" && call .venv\Scripts\activate && python server.py"
)

:: Poll until model is ready (first run downloads ~4 GB, so be patient)
echo [INFO] Waiting for server to be ready (first run downloads Granite model ~4 GB)...
:wait_loop
...
:run
echo [INFO] Server ready. Transcribing with Granite SAA + voiceprints...
