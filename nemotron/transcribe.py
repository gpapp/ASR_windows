...
def get_speech_segments(file_path, max_dur=30):
    ...
    # noise=-35dB: Anything quieter than this is silence.
    # d=2.0: Silence must last at least 2 seconds to be counted.
    command = [
        'ffmpeg', '-i', file_path,
    ...
    # Merge segments that are close (e.g. < 2.5s apart) to keep chunks coherent
    merged = []
    if speech_segments:
        curr_s, curr_e = speech_segments[0]
        for i in range(1, len(speech_segments)):
            next_s, next_e = speech_segments[i]
            if next_s - curr_e < 2.5: 
                curr_e = next_e
            else:
    ...
    for file_path in tqdm(file_paths, desc="Batch Progress"):
        ...
        try:
            for i in tqdm(range(0, num_segments, batch_size), desc="   Processing", leave=False):
                ...
                # Batched Request
                try:
                    response = requests.post(SERVER_URL, json={"wav_paths": to_send}, timeout=120)
